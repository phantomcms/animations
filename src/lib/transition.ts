...
export type AnimationTransitionDictionary = { [key: string]: AnimationMetadata | { [key: string]: AnimationMetadata } };
...
export class AnimationTransitionStore {
  ...
  find(transition: string): AnimationMetadata[] {
    ...
    let result: AnimationTransitionDictionary | AnimationMetadata = this.transitions;
  ...
  set(transition: string, animation: AnimationMetadata[]) {
    ...
    let targetLocation: AnimationTransitionDictionary | AnimationMetadata = this.transitions
    ...
    if (Array.isArray(currentTransitions)) {
      ...
      this.transitions[parts[0]][parts[1]] = currentTransitions.concat(animation);      
    } else {
